...
public class Supporter : MonoBehaviour
{
            ...
            for (int y = 0; y < 9; y++)
            {
                    ...
                    GribManager.cell.GribCellObj[x, y].cell.CellEffect == 0)
                {
                    ...
                    if (obj1 != null)
                    {
                        ...
                        AvaiableObj[0] =
                            JewelSpawner.spawn.JewelGribScript[(int) AvaiableMove[0].x, (int) AvaiableMove[0].y];
                        AvaiableMove[1] = obj1.jewel.JewelPosition;
                        AvaiableObj[1] =
                            JewelSpawner.spawn.JewelGribScript[(int) AvaiableMove[1].x, (int) AvaiableMove[1].y];
                        isNomove = false;
                        return true;
                    }
                }
    ...
    JewelObj MoveChecker(int x, int y, JewelObj obj)
    {
        ...
        {
            if (JewelSpawner.spawn.JewelGribScript[(int) item.x, (int) item.y] != null &&
                JewelSpawner.spawn.JewelGribScript[(int) item.x, (int) item.y].jewel.JewelType == 8)
    ...
    List<Vector2> getListPos(int x, int y)
    {
        ...
        if (y + 1 < 9 && GribManager.cell.GribCellObj[x, y + 1] != null &&
            GribManager.cell.GribCellObj[x, y + 1].cell.CellEffect == 0)
        ...
        if (x + 1 < 7 && GribManager.cell.GribCellObj[x + 1, y] != null &&
            GribManager.cell.GribCellObj[x + 1, y].cell.CellEffect == 0)
